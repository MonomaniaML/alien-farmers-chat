...
Delivery help|ช่วยเหลือเรื่องการจัดส่ง|配送咨询|配送諮詢|Помощь с доставкой
...
Message our team about delivery|ส่งข้อความถึงทีมงานเรื่องการจัดส่ง|向团队咨询配送|向團隊諮詢配送|Написать команде о доставке
Wholesale inquiries for management|สอบถามการขายส่งกับฝ่ายบริหาร|向管理层咨询批发|向管理層諮詢批發|Оптовые вопросы руководству
Staff assistance|เจ้าหน้าที่ให้ความช่วยเหลือ|人工协助|人工協助|Помощь сотрудников
...
Support connected|เชื่อมต่อระบบบริการลูกค้าแล้ว|已连接客服系统|已連接客服系統|Связь с поддержкой установлена
Support unavailable|ระบบบริการลูกค้าไม่พร้อมใช้งาน|客服系统暂时不可用|客服系統暫時無法使用|Поддержка временно недоступна
Coming soon|เร็ว ๆ นี้|即将开放|即將開放|Скоро
Delivery tracking is not available here yet|การติดตามการจัดส่งยังไม่พร้อมใช้งานที่นี่|这里暂不支持配送查询|此處暫不支援配送查詢|Отслеживание доставки здесь пока недоступно
Private feedback is not available here yet|ช่องทางความคิดเห็นส่วนตัวยังไม่พร้อมใช้งานที่นี่|这里暂不支持私密反馈|此處暫不支援私密意見|Личные отзывы здесь пока недоступны
Wholesale inquiries are not available here yet|ช่องทางสอบถามค้าส่งยังไม่พร้อมใช้งานที่นี่|这里暂不支持批发咨询|此處暫不支援批發諮詢|Оптовые запросы здесь пока недоступны
This channel is not live yet. Messages here are not sent to our team.|ช่องทางนี้ยังไม่เปิดใช้งาน ข้อความที่นี่จะไม่ถูกส่งถึงทีมงาน|此频道尚未上线，这里的消息不会发送给团队。|此頻道尚未上線，這裡的訊息不會傳送給團隊。|Этот канал пока не работает. Сообщения здесь не отправляются нашей команде.
Earlier messages here were saved only on this device.|ข้อความก่อนหน้านี้ถูกบันทึกไว้ในอุปกรณ์นี้เท่านั้น|这里以前的消息只保存在此设备。|這裡先前的訊息只儲存在此裝置。|Предыдущие сообщения здесь хранятся только на этом устройстве.
Quick answers on this device; this is not a live AI or staff conversation.|คำตอบด่วนบนอุปกรณ์นี้ ไม่ใช่ AI แบบสดหรือการสนทนากับเจ้าหน้าที่|此设备上的快捷回答，并非实时 AI 或人工客服对话。|此裝置上的快捷回答，並非即時 AI 或人工客服對話。|Быстрые ответы на этом устройстве, а не диалог с ИИ или сотрудником в реальном времени.
Delivery messages go to our team. Order status is not checked automatically.|ข้อความด้านการจัดส่งจะส่งถึงทีมงาน ระบบนี้ไม่ตรวจสอบสถานะคำสั่งซื้ออัตโนมัติ|配送消息会发送给团队；系统不会自动查询订单状态。|配送訊息會傳送給團隊；系統不會自動查詢訂單狀態。|Сообщения о доставке поступают нашей команде. Статус заказа не проверяется автоматически.
Wholesale messages go to management. Prices are not quoted automatically.|ข้อความด้านการขายส่งจะส่งถึงฝ่ายบริหาร ระบบนี้ไม่เสนอราคาอัตโนมัติ|批发消息会发送给管理层；系统不会自动报价。|批發訊息會傳送給管理層；系統不會自動報價。|Оптовые сообщения поступают руководству. Цены не рассчитываются автоматически.
Message sending is not available in this channel.|ช่องทางนี้ยังไม่สามารถส่งข้อความได้|此频道暂不能发送消息。|此頻道暫不能傳送訊息。|Отправка сообщений в этом канале недоступна.
...
Visible only to Owner and Admin|เฉพาะเจ้าของและผู้ดูแลระบบเท่านั้นที่ดูได้|仅 Owner 和 Admin 可查看|僅 Owner 和 Admin 可查看|Видно только владельцу и администратору
...
Connect to send|เชื่อมต่อเพื่อส่ง|连接后发送|連線後傳送|Подключитесь для отправки
