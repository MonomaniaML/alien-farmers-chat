...
export function sharedLocaleCookie(name: 'af_locale' | 'af_locale_override', value: string, request: Request, maxAge: number) { const hostname = new URL(callbackUri(request)).hostname; const domain = hostname.endsWith('.staging.alienfarmers.org') ? '; Domain=.staging.alienfarmers.org' : hostname === 'alienfarmers.org' || hostname.endsWith('.alienfarmers.org') ? '; Domain=.alienfarmers.org' : ''; return `${name}=${encodeURIComponent(value)}; Path=/; SameSite=Lax; Max-Age=${maxAge}${domain}${secureCookie(request) ? '; Secure' : ''}`; }
