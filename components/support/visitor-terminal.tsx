...
function ConversationCenter(){
 const {locale}=useI18n(),copy=(text:string)=>assistantText(text,locale),center=useAssistantCenter(),isDevelopment=process.env.NODE_ENV!=='production',[theme,setTheme]=useState<'dark'|'light'>('dark'),[access,setAccess]=useState<VisitorAccess|null>(null),[gateOpen,setGateOpen]=useState(false),anonymous=access!=='member',candidateId=center.activeId||center.state.lastOpened,candidate=assistantById(candidateId)||ASSISTANTS[0],assistant=anonymous&&!isAnonymousAssistantAvailable(candidate)?ASSISTANTS[0]:candidate,selectedId=assistant.id,conversation=center.state.conversations[assistant.id];
 ...
 function handleSession(next:MemberProfile|null){if(next){setAccess('member');setGateOpen(false);try{localStorage.setItem(accessKey,'member');}catch{}return;}let saved:string|null=null;try{saved=localStorage.getItem(accessKey);}catch{}if(saved==='anonymous')setAccess('anonymous');else{setAccess(null);setGateOpen(true);}}
