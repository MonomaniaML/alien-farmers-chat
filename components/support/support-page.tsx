...
function SupportContent({initialTheme,tickets}:{initialTheme:'dark'|'light';tickets:boolean}){
 ...
 const entries:{title:string;description:string;href:string;icon:ReactNode}[]=[{title:'Service tickets',description:'Ask about a product or request after-sales help.',href:'/tickets',icon:<ClipboardList/>},{title:'Live support',description:'Talk with our support team.',href:'/chat',icon:<MessageCircle/>},{title:'About us',description:'Get to know ALIEN FARMERS.',href:`${platformOrigins.website}/about?af_lang=${locale}`,icon:<Info/>}];
 return <main className={'support-portal theme-'+theme}><PlatformNavigation theme={theme} toggleTheme={toggle} unreadMessageCount={0} onSessionChange={session}/><section className="support-content">{tickets?<><Link className="support-back" href="/"><ArrowLeft size={16}/>{copy('Back to support')}</Link><h1>{copy('Service tickets')}</h1><TicketCenter member={member} ready={ready}/></>:<><p className="support-eyebrow">ALIEN FARMERS</p><h1>{copy('Customer support')}</h1><p className="support-intro">{copy('How can we help?')}</p><div className="support-entry-grid">{entries.map((entry,index)=><Link className="support-entry" key={entry.title} href={entry.href}><span className="support-entry-top">{entry.icon}<small>0{index+1}</small></span><h2>{copy(entry.title)}</h2><p>{copy(entry.description)}</p><ArrowUpRight className="support-entry-arrow"/></Link>)}</div></>}</section></main>;
}
